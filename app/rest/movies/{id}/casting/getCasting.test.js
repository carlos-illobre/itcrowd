...
describe('GET rest/movies/{id}/casting', function() {

  it('return 200 if get the casting', async function() {
    ...
    const { request, body } = await this.request.get(`/rest/movies/${movie.id}/casting`)
      .expect(200)

    const baseUrl = `${request.protocol}//${request.host}/rest`

    expect(body).to.deep.equal({
      items: movie.casting.map(({ id, firstName, lastName }) => ({
        firstName,
        lastName,
        _links: {
          self: {
